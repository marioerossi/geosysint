$(function () {
    if (!search) {
        console.warn("Search parameters not defined!")
        return;
    }
    var searchTextBox = search.searchTextBox;
    var searchList = search.searchList;

    var inputBox = $(searchTextBox);

    var projectsList = $(searchList);
    var projects = $(searchList + " > li");
    var projectsTitles = [];
    ...
    function handleSearch() {
        var search = $.trim(inputBox.val()).substring(0, options.maxPatternLength);
